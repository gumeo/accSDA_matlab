...
B=reshape(A,[],28);
B=transpose(B);
%rewriting first column of Coffee_training matrix
B(:,1)=B(:,1)+1*ones(length(B(:,1)),1);
...
%+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
fid=fopen('Coffee_TEST', 'r');
AT=fscanf(fid,'%g');
fclose(fid);
BT=reshape(AT, [],28);
BT = transpose(BT);
%rewriting first column of Coffee Testing data
BT(:,1) = BT(:,1) + 1*ones(length(BT(:,1)),1);
